...
| `app/Http/Requests/`                    | Form request validation (`StorePostRequest`, `UpdatePostRequest`)              |
| `app/Http/Resources/`                   | `PostResource` — shapes Eloquent models into the JSON the frontend expects     |
| `app/Policies/`                         | `PostPolicy` — view/update/delete authorization rules for posts               |
...
| `MediaController`   | `GET /api/media/{media}`                                       | Streams a post's private image/video with a policy check |
